...
namespace Encompass.MatchSync
{
    ...
    public static class MatchSync
    {
        ...
        public static bool IsMatch<TSource, TDestination>(IEnumerable<TSource> source, IEnumerable<TDestination> destination) where TSource : IMatch, IComparable<TDestination> where TDestination : IMatch
        {
            ...
            if ((from x in sourceArray select (IMatch)x).Except((from x in destinationsArray select (IMatch)x), new MatchComparer()).Any())
                return false;

            if ((from s in sourceArray join d in destinationsArray on s.MatchId equals d.MatchId select new { s, d }).Any(u => u.s.CompareTo(u.d) != 0))
                return false;

            return !(from x in destinationsArray select (IMatch)x).Except((from x in sourceArray select (IMatch)x), new MatchComparer()).Any();
        }
        ...
        public static List<Sync<TSource, TDestination>> Sync<TSource, TDestination>(IEnumerable<TSource> source, IEnumerable<TDestination> destination) where TSource : IMatch, IComparable<TDestination> where TDestination : IMatch
        {
            if (source == null)
            ...
            var result = (from TSource s in (from x in sourceArray select (IMatch)x).Except((from x in destinationArray select (IMatch)x), new MatchComparer()) select new Sync<TSource, TDestination>(s)).ToList();

            result.AddRange((from s in sourceArray join d in destinationArray on s.MatchId equals d.MatchId select new { s, d }).Select(u => new Sync<TSource, TDestination>(u.s, u.d)));

            result.AddRange(from TDestination d in (from x in destinationArray select (IMatch)x).Except((from x in sourceArray select (IMatch)x), new MatchComparer()) select new Sync<TSource, TDestination>(d));
        ...
        /// <returns>A collection of Sync object, which determine what took place during the sync process</returns>
        public static List<Sync<TSource, TDestination>> Sync<TSource, TDestination>(IEnumerable<TSource> source, IList<TDestination> destination, Func<TSource, TDestination> insert, Action<TSource, TDestination> update)
            where TSource : IMatch, IComparable<TDestination> where TDestination : IMatch => Sync(source, destination, insert, update, null);
        ...
        public static List<Sync<TSource, TDestination>> Sync<TSource, TDestination>(IEnumerable<TSource> source,
                                                                                    ...
                                                                                    Action<TDestination> onDeleted) where TSource : IMatch, IComparable<TDestination> where TDestination : IMatch
            => Sync(source, destination, insert, update, null, onDeleted);
