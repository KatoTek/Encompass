...
namespace Encompass.Mvc.Extensions
{
    ...
    public static class ModelStateExtensions
    {
        ...
        public static object SerializeErrors(this ModelStateDictionary modelState) => (from entry in modelState where entry.Value.Errors.Any() select entry).ToDictionary(entry => entry.Key, entry => SerializeModelState(entry.Value));
        ...
        private static string GetErrorMessage(ModelError error, ModelState modelState)
        {
            ...
            return modelState.Value == null ? error.ErrorMessage : Format(CultureInfo.CurrentCulture, ValueNotValidForProperty, modelState.Value.AttemptedValue);
        }
